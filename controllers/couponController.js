...
const claimCoupon = async (req, res) => {
  try {
    ...
    const existingClaim = await Claim.findOne({ $or: [{ ip: userIP }, { browserSession: userSession }] });
    if (existingClaim) {
      return res.status(403).json({ message: "You have already claimed a coupon." });
    }

    // Get the next available coupon (sorted by _id)
    const coupon = await Coupon.findOne({ claimed: false, enabled: true }).sort({ _id: 1 });
    ...
    // Store claim record
    await Claim.create({ coupon: coupon._id, ip: userIP, browserSession: userSession });

    // Set a session cookie for abuse prevention
    res.cookie("session", userSession, { httpOnly: true, maxAge: 24 * 60 * 60 * 1000 }); // 1 day
...
// Function to get all coupons (for admin/testing)
